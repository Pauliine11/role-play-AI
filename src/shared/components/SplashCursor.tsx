...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getSupportedFormat(gl: WebGLRenderingContext | WebGL2RenderingContext | null, internalFormat: number, format: number, type: number): TextureFormat | null {
      if (!gl || !supportRenderTextureFormat(gl, internalFormat, format, type)) {
        if (!gl) return null;
        const gl2 = gl as WebGL2RenderingContext;
    ...
    function supportRenderTextureFormat(gl: WebGLRenderingContext | WebGL2RenderingContext | null, internalFormat: number, format: number, type: number): boolean {
      if (!gl) return false;
    ...
    function initFramebuffers() {
      ...
      if (!rgba || !rg || !r || texType === null || texType === undefined) {
    ...
    function drawDisplay(target: FBO | null) {
      ...
      if (config.SHADING && displayMaterial.uniforms.texelSize) gl.uniform2f(displayMaterial.uniforms.texelSize, 1.0 / width, 1.0 / height);
    ...
    function clickSplat(pointer: Pointer) {
      ...
      const boostedColor = [color[0] * 10.0, color[1] * 10.0, color[2] * 10.0];
      const dx = 10 * (Math.random() - 0.5);
      const dy = 30 * (Math.random() - 0.5);
      splat(pointer.texcoordX, pointer.texcoordY, dx, dy, boostedColor);
    }
